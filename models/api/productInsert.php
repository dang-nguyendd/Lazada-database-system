<?php
    ...
    if ($method == "OPTIONS") {
        ...
        if (!$data || preg_match('/^\s*$/', $data->pName) || preg_match('/^\s*$/', $data->price) || preg_match('/^\s*$/', $data->vendorID)) {
            echo json_encode(
                array("message" => "ERROR: Name, Price, and vendorID must not be blank")
            );
        ...
        $isProductCreated = $productService->createProduct($data->attributes);
        if ($isProductCreated == true) {
            echo json_encode(
                array("message" => "Product inserted successfully.")
            );
